...
class ScrolledText(ttk.Frame):
    ...
    def show_scrollbars(self, *_):
        """Show the scrollbars only if there is content to scroll."""
        try:
            # Get the scroll region for vertical scrollbar
            first, last = self._vbar.get()
            if first != 0.0 or last != 1.0:  # There is content to scroll
                self._vbar.lift(self._text)
        except:
            pass
        
        try:
            # Get the scroll region for horizontal scrollbar
            first, last = self._hbar.get()
            if first != 0.0 or last != 1.0:  # There is content to scroll
